        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-5">
          {STAT_ITEMS.map((stat) => {
            ...
            return (
              <div
                key={stat.label}
                className="trust-badge relative rounded-2xl bg-card p-5 sm:p-6 text-center border border-primary-navy/10 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 ease-out group"
              >
                <div
                  aria-hidden="true"
                  className="mx-auto mb-3 w-12 h-12 rounded-2xl bg-button-gold/15 text-primary-navy flex items-center justify-center group-hover:bg-button-gold/25 transition-colors duration-200 ease-out"
                >
                  <Icon className="w-6 h-6" />
                </div>
                <div className="font-display text-2xl sm:text-3xl lg:text-4xl font-bold text-primary-navy leading-none tracking-tight">
                  {stat.value}
                </div>
